vars <- with(anes2012internet,data.frame(efficacy1stdpre,efficacy1revpre,efficacy2stdpre,efficacy2revpre,efficacy3stdpre,efficacy3revpre,efficacy4stdpre,efficacy4revpre,efficacy1stdpo,efficacy1revpo,efficacy2stdpo,efficacy2revpo,efficacy3stdpo,efficacy3revpo,efficacy4stdpo,efficacy4revpo))

library(ggplot2)
l <- data.frame(matrix(nrow=ncol(vars),ncol=2))
for(i in 1:ncol(vars)){
  a <- hetcor(as.ordered(vars[,i]),as.ordered(anes2012internet$strengthofpid))
...
l <- data.frame(matrix(nrow=ncol(vars),ncol=2))
for(i in 1:ncol(vars)){
  a <- hetcor(as.ordered(vars[,i]),as.numeric(anes2012internet$pctchance))
  j <- c(a$correlations[2,1],a$std.errors[2,1])
  l[i,] <- j
}
l$question <- c("Efficacy 1","Efficacy 1","Efficacy 2","Efficacy 2","Efficacy 3","Efficacy 3","Efficacy 4","Efficacy 4","Efficacy 1","Efficacy 1","Efficacy 2","Efficacy 2","Efficacy 3","Efficacy 3","Efficacy 4","Efficacy 4")
...
l <- data.frame(matrix(nrow=ncol(vars),ncol=2))
for(i in 1:ncol(vars)){
  a <- hetcor(as.ordered(vars[,i]),as.numeric(anes2012internet$polactivity))
  j <- c(a$correlations[2,1],a$std.errors[2,1])
  l[i,] <- j
}
l$question <- c("Efficacy 1","Efficacy 1","Efficacy 2","Efficacy 2","Efficacy 3","Efficacy 3","Efficacy 4","Efficacy 4","Efficacy 1","Efficacy 1","Efficacy 2","Efficacy 2","Efficacy 3","Efficacy 3","Efficacy 4","Efficacy 4")
...
pdf("./output/plotvalidityinternet.pdf",width=12,height=12)
